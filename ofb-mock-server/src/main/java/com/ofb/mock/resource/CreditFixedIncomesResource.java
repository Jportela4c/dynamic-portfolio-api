...
public class CreditFixedIncomesResource {
    ...
    @Operation(summary = "Get investment details", description = "Returns complete details for a specific credit fixed income investment")
    public Response getInvestmentDetails(
            @PathParam("investmentId") String investmentId,
            @HeaderParam("Authorization") String authorization,
            @HeaderParam("x-customer-cpf") String fallbackCpf) { // TEMP: for testing with auth disabled

        log.info("OFB API: GET /open-banking/credit-fixed-incomes/v1/investments/{}", investmentId);
        ...
        CreditFixedIdentification investment = mockDataService.getCreditFixedIncomeDetails(cpf, investmentId);
        if (investment == null) {
    ...
    @Operation(summary = "Get credit fixed income balance", description = "Returns current balance for a specific credit fixed income")
    public Response getInvestmentBalances(
            @PathParam("investmentId") String investmentId,
            @HeaderParam("Authorization") String authorization,
            @HeaderParam("x-customer-cpf") String fallbackCpf) {

        log.info("OFB API: GET /open-banking/credit-fixed-incomes/v1/investments/{}/balances", investmentId);
