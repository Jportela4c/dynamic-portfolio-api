...
public class TreasuryTitlesResource {
    ...
    @Operation(summary = "Get investment details", description = "Returns complete details for a specific treasury title")
    public Response getInvestmentDetails(
            @PathParam("investmentId") String investmentId,
            @HeaderParam("Authorization") String authorization,
            @HeaderParam("x-customer-cpf") String fallbackCpf) { // TEMP: for testing with auth disabled

        log.info("OFB API: GET /open-banking/treasure-titles/v1/investments/{}", investmentId);
        ...
        TreasureTitlesIdentifyProduct investment = mockDataService.getTreasuryTitleDetails(cpf, investmentId);
        if (investment == null) {
    ...
    @Operation(summary = "Get treasury title transactions", description = "Returns transaction history for a specific treasury title")
    public Response getInvestmentTransactions(
            @PathParam("investmentId") String investmentId,
            @HeaderParam("Authorization") String authorization,
            @HeaderParam("x-customer-cpf") String fallbackCpf) { // TEMP: for testing with auth disabled

        log.info("OFB API: GET /open-banking/treasure-titles/v1/investments/{}/transactions", investmentId);
    ...
    @Operation(summary = "Get treasury balance", description = "Returns current balance for a specific treasury title")
    public Response getInvestmentBalances(
            @PathParam("investmentId") String investmentId,
            @HeaderParam("Authorization") String authorization,
            @HeaderParam("x-customer-cpf") String fallbackCpf) {

        log.info("OFB API: GET /open-banking/treasure-titles/v1/investments/{}/balances", investmentId);
